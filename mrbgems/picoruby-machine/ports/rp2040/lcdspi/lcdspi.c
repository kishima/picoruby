...
void define_region_spi(int xstart, int ystart, int xend, int yend, int rw) {
    ...
    // Memory Write/Read Command
    if (rw) {
        spi_write_command(ILI9341_MEMORYWRITE);
    } else {
        spi_write_command(ILI9341_RAMRD);
    }
...
void draw_rect_spi(int x1, int y1, int x2, int y2, int c) {
    ...
    if (x1 == x2 && y1 == y2) {
        ...
        define_region_spi(x1, y1, x2, y2, 1);
#ifdef ILI9488
        col[0] = (c >> 16) & 0xFF;  // Red → 送信順序1番目
        col[1] = (c >> 8) & 0xFF;   // Green → 送信順序2番目
        col[2] = (c & 0xFF);        // Blue → 送信順序3番目
#endif
        hw_send_spi(col, 3);
    } else {
        int i, t, y;
...
void pico_lcd_init() {
    ...
    spi_write_data(0xE8); // MX=1, MY=1, MV=1, BGR=1 (X軸Y軸反転+行列交換+BGR)
